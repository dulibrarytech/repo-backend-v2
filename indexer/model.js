...
async function mark_indexed(pid) {
    require_pid(pid);
    await db()(OBJECTS).where({ pid }).update({ is_indexed: 1 });
}
...
async function mark_deindexed(pid) {
    require_pid(pid);
    await db()(OBJECTS).where({ pid }).update({ is_indexed: 0 });
}
...
async function mark_dirty_pid(pid) {
    require_pid(pid);
    const affected = await db()(OBJECTS).where({ pid }).update({ is_updated: 1 });
    return { affected };
}
...
async function reset_all_index_state() {
    const affected = await db()(OBJECTS)
        .where({ is_active: 1, is_published: 1 })
        .update({ is_updated: 1, is_indexed: 0 });
    return { affected };
}
...
async function mark_indexed_bulk(pids) {
    ...
    const affected = await db()(OBJECTS).whereIn('pid', pids).update({ is_indexed: 1 });
    return { affected };
}

async function mark_deindexed_bulk(pids) {
    if (!Array.isArray(pids) || pids.length === 0) return { affected: 0 };
    const affected = await db()(OBJECTS).whereIn('pid', pids).update({ is_indexed: 0 });
    return { affected };
}
